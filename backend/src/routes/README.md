...
**Périodes de possession** : les deux familles de périodes (console et jeu+plateforme) exposent le même triplet d'écritures `POST` / `PUT` / `DELETE`. Le `PUT` existe spécifiquement pour le volet financier : le prix de vente et l'acheteur ne sont connus qu'au moment de la revente, donc bien après la création de la période d'achat. La normalisation des champs financiers est déléguée à `services/ownershipFinancials.js` pour que les deux routes restent strictement alignées. `GET /api/games?console_id=` reste le principal point d'entrée du frontend (une ligne par jeu ayant une instance sur cette plateforme) ; sans `console_id`, l'endpoint retourne une vue agrégée (heures cumulées toutes plateformes par jeu).
